...
fn main() -> Result<(), Box<Error>> {
    ...
    if args.len() != 5 {
        println!(
            "invalid # of args: {}.  usage: gundam_converge <motifs.txt> <pos.fa> <neg.fa> <threshold>",
            args.len()
        );
        exit(1);
    }
    ...
    let threshold: f32 = args[4]
        .parse::<f32>()
        .expect("fourth argument should be a float value representing threshold");
        ...
        for window_num in 0..KMER_SPLIT {
            ...
            if final_pval != -1.0 {
                ...
                let (pval, _) = seqs_to_dyad(
                    ...
                    neg.len(),
                    Some(threshold), //passing_threshold(&motif)),
                );
                scores
